...
!function ($) {
    ...
    var DateRangePicker = function (element, options, cb) {
        ...
        this.container.find('.ranges').on('click', 'a.thisWeekBtn', $.proxy(this.clickThisWeek, this));
        this.container.find('.ranges').on('click', 'a.thisMonthBtn', $.proxy(this.clickThisMonth, this));
    ...
    DateRangePicker.prototype = {
        ...
        clickThisWeek: function (e) {
        	var now = new Date();
        	var day = now.getDay();
        	var weekBegin = new Date(now.getTime() - day*24*60*60*1000);
        	var weekEnd = new Date(now.getTime() + (6 - day)*24*60*60*1000);
        	$('input[name="reservation"]').val(weekBegin.toString('dd/MM/yyyy') + ' - ' + weekEnd.toString('dd/MM/yyyy'));
        	this.hide();
        },
        ...
        clickThisMonth: function (e) {
        	var now = new Date();
        	var day = now.getDate();
        	var weekBegin = new Date(now.getTime() - (day-1)*24*60*60*1000);
        	
        	var weekEndTmp = new Date();
        	if (now.getMonth() == 11) {
        		weekEndTmp = new Date(now.getFullYear() + 1, 0, 1);
        	} else {
        		weekEndTmp = new Date(now.getFullYear(), now.getMonth() + 1, 1);
        	}
        	day = weekEndTmp.getDate();
        	var weekEnd = new Date(weekEndTmp.getTime() - day*24*60*60*1000);
        	$('input[name="reservation"]').val(weekBegin.toString('dd/MM/yyyy') + ' - ' + weekEnd.toString('dd/MM/yyyy'));
        	this.hide();
        },
